...
interface SaveCancelButtonsSectionProps {
  closeModal: () => void;
  currentDay: number;
  ...
  handleDayChange: (dayIndex: number) => void;
  isExcursion: boolean;
  isWedding: boolean;
  restHours?: RestHours;
  allStops: StopData[];
...
const SaveCancelButtonsSection = ({
  closeModal,
  currentDay,
  pickupLocation,
  dropoffLocation,
    ...
    <div className="w-full flex flex-col lg:flex-row justify-center items-center space-x-0 space-y-2 lg:space-y-0 lg:space-x-4 mt-6">
      ...
      {isTrip && currentDay !== numDays - 1 ? (
        <Button
          ...
          size={'xl'}
          disabled={!isDayFullyFilled(currentDay, dailyStops)}
          onClick={() => handleDayChange(currentDay + 1)}
          className="w-full md:w-auto"
        >
          {t('admin.buttons.nextDay')}
        </Button>
      ) : (
        <Button
          variant="default"
...
export default SaveCancelButtonsSection;
